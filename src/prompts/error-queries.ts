...
export const errorQueries: Record<string, NrqlQuery> = {
	...
	errorLogs: {
		query:
			"SELECT `tag`,`message`,`userAgent`,`component` FROM Log WHERE `svc` = '${serviceName}' AND `env` = '${environment}' AND `responseCode` >= 500 SINCE ${timeframe} AGO",
		description:
			"Shows the error logs with details like tag, message, userAgent, and component. This is useful to identify errors and can be used to get the trace and visualize the errors.",
		variables: {
			serviceName:
				"The name of the service to analyze errors for (specified by the 'svc' attribute)",
			environment:
				"The environment to analyze (specified by the 'env' attribute, e.g., 'prod', 'dev', 'staging')",
			timeframe:
				"The time period to analyze (e.g., '1 hour', '1 day', '7 days')",
		},
		interpretation:
			"Results show detailed error logs with HTTP 500+ status codes. Examine the message field for error details, tag for categorization, userAgent to identify client types, and component to pinpoint which part of the service is failing.",
	},
};
