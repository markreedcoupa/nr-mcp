...
export const exploreErrorsPrompt = {
	...
	arguments: [
		...
		},
		{
			name: "environment",
			description:
				"The environment to analyze (specified by the 'env' attribute, e.g., 'prod', 'staging')",
			required: false,
		},
		{
			name: "timeframe",
			description:
				"The time period before now to analyze (e.g., '1 hour', '1 day')",
			required: false,
		},
		{
			name: "queryType",
...
export async function handleExploreErrorsPrompt(args: Record<string, unknown>) {
	...
	const selectedQuery: NrqlQuery =
		errorQueries[queryType] || errorQueries.errorLogs;
